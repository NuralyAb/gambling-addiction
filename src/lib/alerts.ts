...
export async function sendAlert(userId: string, reason: string) {
  ...
  if (!user?.trusted_person_chat_id) {
    return { error: "Доверенное лицо не подключено" };
  }
...
export async function checkAndAlert(userId: string): Promise<void> {
...
async function sendAlertDirect(userId: string, reason: string): Promise<void> {
  const { data: user } = await supabase
    .from("users")
