...
export default function Navbar() {
  ...
  return (
    <nav className="border-b border-white/10 bg-dark/70 backdrop-blur-xl sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between gap-2 min-w-0">
        <Link href="/" className="flex items-center gap-2 shrink-0 min-w-0">
          ...
          <span className="font-display font-semibold text-base sm:text-lg text-white truncate">SafeBet AI</span>
        </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          {session ? (
            <>
              <Link href="/dashboard">
                <Button variant="ghost" size="sm" className="!px-2 sm:!px-3">Дашборд</Button>
              </Link>
              <Link href="/profile">
                <Button variant="ghost" size="sm" className="!px-2 sm:!px-3">Профиль</Button>
              </Link>
