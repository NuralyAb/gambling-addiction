...
export async function POST(req: Request) {
  ...
  // Verify this tg is actually the trusted person for this user
  ...
  if (user.tg_chat_id) {
    await sendTelegramMessage(
      user.tg_chat_id,
      "✅ Доверенное лицо одобрило вашу разблокировку на 24 часа."
    );
  }

  return NextResponse.json({ success: true, message: "Разблокировка одобрена" });
}
